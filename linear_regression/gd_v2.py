...
def gradient_descent(X,y,theta,lr, maxIrr,m):
...
def generate_data(num_samples):
    ...
    y=np.dot(X,theta0)+random.random()* random.randint(-1, 1)
...
def train(sample_X, sample_y, batch_size,lr, maxIrr):
    ...
    for i in range (maxIrr):
        ...
        theta,J=gradient_descent(batch_X, batch_y, theta, lr, maxIrr, batch_size)
